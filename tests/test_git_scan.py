...
class TestScanUnstaged:
    def test_detects_ai_trace_in_unstaged_changes(self, git_repo):
        f = git_repo / "file.txt"
        f.write_text("initial\n")
        _git_run("add", ".", cwd=git_repo)
        _git_run("commit", "-m", "init", cwd=git_repo)
        f.write_text("initial\n# generated by copilot\n")
        findings = scan_unstaged(git_repo, lambda _: False)
        assert len(findings) >= 1
        assert any(f.category == "unstaged-change" for f in findings)

    def test_ignores_staged_changes(self, git_repo):
        f = git_repo / "file.txt"
        f.write_text("initial\n")
        _git_run("add", ".", cwd=git_repo)
        _git_run("commit", "-m", "init", cwd=git_repo)
    ...
    def test_clean_unstaged_produces_no_findings(self, git_repo):
        f = git_repo / "file.txt"
        f.write_text("initial\n")
        _git_run("add", ".", cwd=git_repo)
        _git_run("commit", "-m", "init", cwd=git_repo)
    ...
    def test_exclude_fn_filters_unstaged(self, git_repo):
        f = git_repo / "file.txt"
        f.write_text("initial\n")
        _git_run("add", ".", cwd=git_repo)
        _git_run("commit", "-m", "init", cwd=git_repo)
        f.write_text("initial\n# generated by copilot\n")
        findings = scan_unstaged(git_repo, lambda s: "file.txt" in s)
        assert len(findings) == 0
